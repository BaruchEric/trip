...
export async function removeSegment(
  ...
  // Placements reference segments, so the derived row goes first.
...
describe("clusterSegments", () => {
  ...
  test("respects capacity by rebalancing into a neighbour", () => {
    const { clusters, overflow } = clusterSegments([...ALFAMA, ...BELEM], 2, 2);
    expect(overflow).toHaveLength(0);
...
describe("orderDay", () => {
  ...
  test("beats the naive input order on total transit", () => {
    ...
    // Input order a -> c -> b walks the span 1.5x; the sweep walks it once.
    expect(ordered.transitMinutes).toBeLessThan(30);
  });
...
Expected: M1 behaviour is unchanged against `~/.trip/trip.db`, now at schema version 4.
